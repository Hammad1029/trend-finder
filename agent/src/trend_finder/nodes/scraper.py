...
from trend_finder.schemas import ProductMetrics
from trend_finder.services.external import ApifyService
from trend_finder.services.scoring import calculate_product_score
from trend_finder.llm import get_embeddings_model
from trend_finder.database import get_db, ProductMetricsDB
    ...
    with get_db() as session:
        db_products = [
            ProductMetricsDB(
                ...
                platform=(
                    p.platform.value if hasattr(p.platform, "value") else p.platform
                ),
                unique_id=p.unique_id,
                description=p.description,
                price=p.price,
                currency=(
                    p.currency.value if hasattr(p.currency, "value") else p.currency
                ),
