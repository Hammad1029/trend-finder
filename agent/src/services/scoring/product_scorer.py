...
from config.constants import PRODUCT_SCORER_CONFIG, ProductScorerConfig
...
class ProductScorer:
    ...
    def _calculate_velocity(self, product: ProductLike) -> float:
        ...
        velocity_ratio = (
            product.sales_last_month / (product.review_count + self.config.REVIEW_SMOOTHER)
    ...
    def _calculate_friction(self, product: ProductLike) -> float:
        ...
        return self._weigh_score(friction_rank + friction_flaws, self.config.FRICTION_WEIGHT)
